...
export default function Home() {
  ...
  return (
    ...
    <Layout>
      ...
      {/* Mobile section navigation - Only visible on smaller screens */}
      <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-black/70 backdrop-blur-lg border-t border-accent/20">
        <div className="flex justify-around items-center py-2">
          {['hero', 'about', 'services', 'testimonials', 'cta'].map((section) => (
            <button
              key={section}
              onClick={() => scrollToSection(section)}
              className={`flex flex-col items-center px-2 py-1 ${
                activeSection === section ? 'text-accent' : 'text-white/60'
              }`}
              aria-label={`Navigate to ${section} section`}
            >
              {section === 'hero' && (
                <svg className="w-5 h-5 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1h2a1 1 0 001-1v-4a1 1 0 00-1-1h-4a1 1 0 00-1 1v4a1 1 0 01-1 1H9" />
                </svg>
              )}
              {section === 'about' && (
                <svg className="w-5 h-5 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <span className="text-xs">{section === 'hero' ? 'Home' : section.charAt(0).toUpperCase() + section.slice(1)}</span>
              {activeSection === section && <div className="w-1.5 h-1.5 rounded-full bg-accent mt-1"></div>}
            </button>
          ))}
        </div>
      </div>
